      ...
      if (wiFI.CheckMQTTConnection()) {
        ...
        if ((currentTime - previousTime >= (interval * 1000)) || responseOn200Request) {
          if (responseOn200Request) {
            wiFI.publishMqttMsg(Topic.Publish, "300", powerData.voltage, powerData.current, powerData.power, relayLoadStaus);
            responseOn200Request = false;
          } else {
            wiFI.publishMqttMsg(Topic.Publish, deviceId, powerData.voltage, powerData.current, powerData.power, relayLoadStaus);
